...
import { MAX_CUSTOMER_CONTACTS, type CustomerContact } from "@/lib/crm/types";
import { Card, CardHeader } from "@/components/ui/Card";
import { inputClass } from "@/components/ui/Form";

const EMPTY: CustomerContactInput = { name: "", role: "", phone: "", email: "", notes: "" };
...
export function CustomerContacts({
  ...
  const full = contacts.length >= MAX_CUSTOMER_CONTACTS;

  return (
    <Card>
      <CardHeader
        title={`Personas de contacto (${contacts.length}/${MAX_CUSTOMER_CONTACTS})`}
        action={
          !adding && !full ? (
            <button
              onClick={() => {
      ...
      {full && !adding && (
        <p className="border-b border-ink-100 bg-amber-50 px-4 py-2 text-xs text-amber-700">
          Has alcanzado el máximo de {MAX_CUSTOMER_CONTACTS} contactos. Elimina uno para añadir otro.
        </p>
      )}
      ...
      {contacts.length === 0 && !adding ? (
        <div className="flex flex-col items-center justify-center px-6 py-8 text-center">
          ...
          <p className="mt-2 text-sm text-ink-400">
            Aún no hay personas de contacto. Puedes añadir hasta {MAX_CUSTOMER_CONTACTS}.
          </p>
        </div>
      ) : (
            ...
            editingId === c.id ? (
              ...
              <li key={c.id} className="flex items-start justify-between gap-3 p-4">
                ...
                <div className="flex shrink-0 items-center gap-1">
                  ...
                  <DeleteContact id={c.id} customerId={customerId} name={c.name} />
                </div>
              </li>
            ),
          )}
...
function ContactForm({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setError(null);
    const res = contact
      ? await updateCustomerContact(contact.id, customerId, form)
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-2">
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        ...
        <input className={inputClass} value={form.role ?? ""} onChange={(e) => set("role", e.target.value)} placeholder="Cargo (ej. Administración)" />
...
function DeleteContact({ id, customerId, name }: { id: string; customerId: string; name: string }) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  async function onDelete() {
    if (!window.confirm(`¿Eliminar el contacto "${name}"?`)) return;
    setLoading(true);
    const res = await deleteCustomerContact(id, customerId);
    if (!res.ok) {
      setLoading(false);
